        ...
        <a-tab-pane v-for="tab in tabs" :key="tab.key" :tab="tab.title">
          <div class="news-list">
            ...
            <div v-else-if="newsList.length === 0" class="empty-container">
              <a-empty style="margin-top: -5rem" description="GET READY" />
              <rocket-filled style="font-size: 2rem" />
            </div>
            <div class="news-item" v-for="news in newsList" :key="news.id" @click="handleDetail(news.id)">
              ...
              <div class="news-content">
                ...
                <div class="news-item-footer">
                  ...
                  <a-button type="link" size="small" @click.stop="handleDetail(news.id)">
                    {{ $t('moo.about.detail') }}
                  </a-button>
                </div>
              </div>
...
const handleDetail = (id: number) => {
  emit('detail', id);
  // 在新窗口打开详情页面
  window.open(`/about-us/blog-banner/detail/${id}`, '_blank');
};
